...
| 终端与工具 | 内置 xterm.js 终端、工具调用可视化、上下文/附件细节查看、工具输出支持按只读/非只读自动折叠 |
...
| 隐藏项目状态可持久化 | 在侧边栏隐藏项目后，刷新页面或重新进入应用仍会保持隐藏；再次打开该项目时会自动恢复显示 |
...
| 工具输出可分级折叠 | 支持 4 级自动展开策略，可按只读工具、简单只读 bash 命令和自定义通配符控制默认折叠行为 |
| 工具意图提前展示 | 工具还没执行完成时，也会先显示工具名称、目标路径、搜索词或操作意图，方便及时判断 |
| 待授权工具自动高亮 | 当前正在等待授权的工具调用会自动展开并高亮，便于先看清 diff / 参数再决定是否放行 |
| 可从历史消息直接分叉会话 | 在任意用户消息处可直接从该节点 fork 出新会话，方便基于某一步尝试不同思路 |
...
| 优化点 | 说明 |
|--------|------|
| 自动授权范围更贴近 Web 行为 | `Always allow` 不再只是当前页面内的临时状态，而是支持按会话血缘和项目目录继承，刷新后也能保留 |
| 权限操作栏信息更完整 | 底部授权条会优先展示工具名、操作意图、目标路径和排队数量，降低“看不懂该不该放行”的成本 |
| 工具输出折叠策略可配置 | 设置里新增 4 级工具输出展开策略，还支持为只读 bash 命令补充自定义通配规则 |
