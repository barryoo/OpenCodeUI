...
工具输出支持按配置自动展开或折叠，默认使用第 3 级策略：

- 只读工具（如 Read、Grep、Glob）默认折叠
- 非只读工具（如写文件、补丁、执行命令）默认展开
- 对简单 bash 命令会额外判断是否属于只读命令；复杂命令不做解析，避免误判
- 用户可以在设置中切换 4 个级别，并为第 3 级补充自定义只读命令通配符
