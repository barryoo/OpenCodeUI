...
const THIN_SERVER_BASE_PATH = '/admin'
...
function stripThinServerBasePath(pathname: string): string {
  if (pathname === THIN_SERVER_BASE_PATH) return '/'
  if (pathname.startsWith(`${THIN_SERVER_BASE_PATH}/`)) {
    return pathname.slice(THIN_SERVER_BASE_PATH.length)
  }
  return pathname
}
...
export async function handleRequest(request: Request, context: AppContext): Promise<Response> {
  ...
  const thinPathname = stripThinServerBasePath(pathname)
  ...
  if ((pathname === `${THIN_SERVER_BASE_PATH}/meta` || thinPathname === '/meta') && method === 'GET') {
  ...
  if (thinPathname === '/auth/github/login' && method === 'GET') {
    if (context.config.githubClientId && context.config.githubClientSecret) {
      ...
      const redirectUri = `${getPublicBaseUrl(request, context.config.publicBaseUrl)}${THIN_SERVER_BASE_PATH}/auth/github/callback`
  ...
  if (thinPathname === '/auth/github/callback' && method === 'GET') {
  ...
  if (thinPathname === '/server-profiles' && method === 'GET') {
    return withCors(json({ data: context.repository.listServerProfiles(user.id) }), request)
  }

  if (thinPathname === '/server-profiles' && method === 'POST') {
    const body = await parseJsonBody<Record<string, unknown>>(request)
    if (!body) return withCors(errorResponse(400, 'INVALID_JSON', 'Request body must be valid JSON'), request)
  ...
  const serverProfileMatch = thinPathname.match(/^\/server-profiles\/([^/]+)$/)
  ...
  const serverProfileDefaultMatch = thinPathname.match(/^\/server-profiles\/([^/]+)\/default$/)
  ...
  if (thinPathname === '/items' && method === 'GET') {
    const projectPath = url.searchParams.get('projectPath') ?? url.searchParams.get('projectId')
    const legacyProjectId = url.searchParams.get('legacyProjectId')
    const data = projectPath ? context.repository.listItems(user.id, projectPath, legacyProjectId) : context.repository.listItemsByUser(user.id)
    return withCors(json({ data }), request)
  }

  if (thinPathname === '/items' && method === 'POST') {
    const body = await parseJsonBody<Record<string, unknown>>(request)
    if (!body) return withCors(errorResponse(400, 'INVALID_JSON', 'Request body must be valid JSON'), request)

    const serverProfileId = asString(body.serverProfileId)
    const projectPath = asString(body.projectPath) ?? asString(body.projectId)
    const legacyProjectId = asString(body.legacyProjectId) ?? asString(body.projectId)
    const title = asString(body.title)
    const type = asItemType(body.type)
    if (!serverProfileId || !projectPath || !title || !type) {
      return withCors(errorResponse(400, 'VALIDATION_ERROR', 'serverProfileId, projectPath, title, type are required'), request)
    }
  ...
  const itemMatch = thinPathname.match(/^\/items\/([^/]+)$/)
  ...
  const itemSessionsMatch = thinPathname.match(/^\/items\/([^/]+)\/session-summaries$/)
  ...
  if (thinPathname === '/session-summaries' && method === 'GET') {
    const projectPath = url.searchParams.get('projectPath') ?? url.searchParams.get('projectId')
    const legacyProjectId = url.searchParams.get('legacyProjectId')
    const data = projectPath ? context.repository.listSessionSummaries(user.id, projectPath, legacyProjectId) : context.repository.listSessionSummariesByUser(user.id)
    return withCors(json({ data }), request)
  }

  if (thinPathname === '/session-summaries' && method === 'POST') {
    const body = await parseJsonBody<Record<string, unknown>>(request)
    if (!body) return withCors(errorResponse(400, 'INVALID_JSON', 'Request body must be valid JSON'), request)
    ...
    if (!serverProfileId || !projectPath || !externalSessionId || !titleSnapshot || !statusSnapshot || !activityAt) {
      return withCors(errorResponse(400, 'VALIDATION_ERROR', 'serverProfileId, projectPath, externalSessionId, titleSnapshot, statusSnapshot, activityAt are required'), request)
    }
    ...
    const input: UpsertSessionSummaryInput = {
      ...
      legacyProjectId,
      externalSessionId,
      variant: asNullableString(body.variant),
      titleSnapshot,
      statusSnapshot,
      activityAt,
  ...
  const sessionMatch = thinPathname.match(/^\/session-summaries\/([^/]+)$/)
  ...
  const bindMatch = thinPathname.match(/^\/session-summaries\/([^/]+)\/bind-item$/)
  ...
  const unbindMatch = thinPathname.match(/^\/session-summaries\/([^/]+)\/unbind-item$/)
  ...
  const projectItemsMatch = thinPathname.match(/^\/projects\/([^/]+)\/items$/)
  if (projectItemsMatch && method === 'GET') {
    const projectPath = decodeURIComponent(projectItemsMatch[1])
    const legacyProjectId = url.searchParams.get('legacyProjectId')
    return withCors(json({ data: context.repository.listItems(user.id, projectPath, legacyProjectId) }), request)
  }

  const projectSessionsMatch = thinPathname.match(/^\/projects\/([^/]+)\/session-summaries$/)
  if (projectSessionsMatch && method === 'GET') {
    const projectPath = decodeURIComponent(projectSessionsMatch[1])
    const legacyProjectId = url.searchParams.get('legacyProjectId')
    return withCors(json({ data: context.repository.listSessionSummaries(user.id, projectPath, legacyProjectId) }), request)
  }

  const projectSearchMatch = thinPathname.match(/^\/projects\/([^/]+)\/files\/search$/)
  if (projectSearchMatch && method === 'GET') {
    const q = url.searchParams.get('q')?.trim() ?? ''
    if (!q) return withCors(errorResponse(400, 'VALIDATION_ERROR', 'q is required'), request)
    const projectPath = decodeURIComponent(projectSearchMatch[1])
    const legacyProjectId = url.searchParams.get('legacyProjectId')
    return withCors(json({ data: context.repository.searchProjectFiles(user.id, projectPath, q, legacyProjectId) }), request)
  }
